...
export class CrossTabClient extends Client {
    constructor (opts = {}) {
        ...
        if (this.isLocalStorage) {
            const subprotocolKey = storageKey(this, 'subprotocol')
    ...
    getClientId () {
        const key = storageKey(this, 'client')
        if (!this.isLocalStorage) {
    ...
    onStorage (e) {
        ...
        if (e.key === storageKey(this, 'add')) {
            ...
            const state = JSON.parse(localStorage.getItem(e.key))
            ...
            const compare = compareSubprotocols(this.options.subprotocol, other)
            if (compare === 1) {
                sendToTabs(this, 'subprotocol', this.options.subprotocol)
            } else if (compare === -1) {
                const err = new LoguxError(
                    'wrong-subprotocol',
    ...
    start (connect = true) {
      ...
      !this.isLocalStorage
        ...
        const json = localStorage.getItem(storageKey(this, 'state'))
